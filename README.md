...
- Command-line arguments are saved on the stack and accessed using the *arg(i)* function, where *i* is the index of the argument.
  To support minimalism, the same *arg(i)* function is used to read also function arguments inside scripted functions.
- Script execution is non-blocking, meaning you can execute the script line by line, doing other things in between, as the Paula engine maintains its execution state.
  ...
  Procedures (which do not return values) are a non-blocking alternative for scripted functions. See examples for syntax.
- Paula script functionality is implemented in C++ callbacks.
